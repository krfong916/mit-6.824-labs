...
// 				for j := 0; j < len(cfg.logs); j++ {
// 					if old, oldok := cfg.logs[j][m.CommandIndex]; oldok && old != v {
// 						color.New(color.FgRed).Printf("DEBUG: PEER[%v]: {term: %v, value: %v}\n", i, m.CommandIndex, v)
// 						color.New(color.FgRed).Printf("DEBUG: PEER[%v]: log: %v\n", j, cfg.logs[j])
// 						color.New(color.FgRed).Printf("DEBUG: comparing old=%v to v=:%v\n", old, v)
// 						fmt.Println("")
// 						// some server has already committed a different value for this entry!
