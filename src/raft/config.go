			...
			if m.CommandValid == false {
				...
				for j := 0; j < len(cfg.logs); j++ {
					if old, oldok := cfg.logs[j][m.CommandIndex]; oldok && old != v {
						color.New(color.FgRed).Printf("DEBUG: peer i: %v\n", i)
						color.New(color.FgRed).Printf("DEBUG: message: %v\n", m)
						color.New(color.FgRed).Printf("DEBUG: term: %v\n", m.CommandIndex)
						color.New(color.FgRed).Printf("DEBUG: value: %v\n", v)

						color.New(color.FgRed).Printf("DEBUG: peer j: %v\n", j)
						color.New(color.FgRed).Printf("DEBUG: logs[j]: %v\n", cfg.logs[j])
						color.New(color.FgRed).Printf("DEBUG: old value: %v\n", old)
						color.New(color.FgRed).Printf("DEBUG: old value is applied: %v\n", oldok)
						fmt.Println("")
						// some server has already committed a different value for this entry!
						err_msg = fmt.Sprintf("commit index=%v for server=%v has value=%v != server=%v value=%v",
